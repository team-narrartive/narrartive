...
BEGIN
  ...
  VALUES (auth.uid(), increment_story_likes.story_id)
  ...
  IF FOUND THEN
    ...
    SET like_count = COALESCE(like_count, 0) + 1,
        updated_at = now()
    WHERE public.stories.id = increment_story_likes.story_id;
  END IF;
...
-- Recreate decrement_story_likes: only decrement if actually unliked, bypass RLS with SECURITY DEFINER (fixed syntax)
...
AS $$
BEGIN
  -- Delete like record (only if it exists and belongs to caller) -- Fixed parameter reference
